...
-- (1) 주문 테이블(tbl_order)에 insert 후 (2) 주문 메뉴 테이블(tbl_order_menu)에 주문한 메뉴마다 insert
-- (3) 그 후 주문 테이블에 총 금액이 업데이트 되는지 확인
...
, CONCAT(CAST(YEAR(NOW()) AS VARCHAR(4))
	    , CAST(LPAD(MONTH(NOW()), 2, 0) AS VARCHAR(2))  -- now(날짜) -> month(날짜) -> lpad(문자) -> cast(varchar) 형으로 명시적 형변환
...
CREATE TABLE PRODUCT(           -- 이력 테이블(UPDATE가 발생하는 테이블) -> 트리거로 값을 자동으로 업데이트할 테이블
  PCODE INT PRIMARY KEY auto_increment,
  PNAME VARCHAR(30),
  BRAND VARCHAR(30),
  PRICE INT,
  STOCK INT DEFAULT 0,
  CHECK(STOCK >= 0)  -- 출고 재고가 -가 되지않도록 함.
);
...
-- -------------------------------------------------------------
-- 입출고 발생시 product table의 재고를 자동으로 관리하는 트리거 생성
DELIMITER //
